...
class SlackFilterSpecTests(unittest.TestCase):
    def test_construct(self):
        with self.assertRaisesRegex(
            slack.SlackConfigError, "Empty filter spec for 'foo'"
        ):
        ...
        self.assertEqual(
            re.compile("Hi"),
            slack.SlackFilterSpec('msg=Hi', 'foo', ',', '=').message_regex,
        )
        self.assertEqual(
            [], slack.SlackFilterSpec('msg=Hi', 'foo', ',', '=').arg_regexes
        )
        self.assertEqual(
            {}, slack.SlackFilterSpec('msg=Hi', 'foo', ',', '=').extra_regexes
        )
        self.assertEqual(
        ...
        self.assertEqual(
            [None, re.compile('b'), None, re.compile('d')],
            slack.SlackFilterSpec(
                'msg=Hi,1=b,3=d', 'foo', ',', '='
            ).arg_regexes,
        )
        self.assertEqual(
            {'a': re.compile('1'), 'b': re.compile('2')},
            slack.SlackFilterSpec(
                'msg=Hi,a=1,b=2', 'foo', ',', '='
            ).extra_regexes,
        )
    ...
    def test_filter(self):
        ...
        record = TEST_LOG.makeRecord(
            'a.b.c', logging.INFO, "code.py", 42, "Hi", [], None
        )
        self.assertTrue(spec.filter(record))
        record = TEST_LOG.makeRecord(
            'a.b.c', logging.INFO, "code.py", 42, "Hi", [1, 2, 3], None
        )
        self.assertTrue(spec.filter(record))
        record = TEST_LOG.makeRecord(
            'a.b.c', logging.INFO, "code.py", 42, "Ho", [1, 2, 3], None
        )
        ...
        self.assertTrue(spec.filter(record))
        record = TEST_LOG.makeRecord(
            'a.b.c', logging.INFO, "code.py", 42, "Hi", [0, 2, 3], None
        )
        self.assertFalse(spec.filter(record))
        record = TEST_LOG.makeRecord(
            'a.b.c', logging.INFO, "code.py", 42, "Hi", [1], None
        )
        self.assertFalse(spec.filter(record))
        record = TEST_LOG.makeRecord(
            'a.b.c', logging.INFO, "code.py", 42, "Hi", [1, None], None
        )
        self.assertFalse(spec.filter(record))
...
class SlackConfigTests(unittest.TestCase):
    ...
    def test_configure(self, mock_get, mock_isfile):
        ...
        self.assertEqual([re.compile("a")], filter_0.arg_regexes)
        self.assertEqual(
            {'other': re.compile("other")}, filter_0.extra_regexes
        )
        self.assertIsNone(filter_0.alternative_message)
        self.assertEqual(re.compile("Ho"), filter_1.message_regex)
        self.assertEqual([None, re.compile("b")], filter_1.arg_regexes)
        self.assertEqual(
            {'other': re.compile("other")}, filter_1.extra_regexes
        )
        self.assertIsNone(filter_1.alternative_message)
        self.assertEqual(re.compile("Hum"), filter_2.message_regex)
        self.assertEqual([re.compile("c")], filter_2.arg_regexes)
        self.assertEqual(
            {'other': re.compile("other")}, filter_2.extra_regexes
        )
    ...
    def test_handler(self, mock_isfile):
        ...
        with managed_config(config, "allocations"):
            ...
            with unittest.mock.patch.object(config.handlers[0], 'emit') as m:
                my_logger.warning(
                    "Hi: %s", "a", extra={'extra': {'other': 'other'}}
                )
                m.assert_called_once()

                m.reset_mock()
                my_logger.warning(
                    "Ho: %s, %s", "x", "b", extra={'extra': {'other': 'other'}}
                )
                m.assert_called_once()
                ...
                m.reset_mock()
                my_logger.warning(
                    "Bye: %s", "a", extra={'extra': {'other': 'other'}}
                )
                m.assert_not_called()

                m.reset_mock()
                my_logger.warning(
                    "Hi: %s", "b", extra={'extra': {'other': 'other'}}
                )
                m.assert_not_called()

                m.reset_mock()
                my_logger.warning(
                    "Bye: %s", "a", extra={'extra': {'other': 'same'}}
                )
                m.assert_not_called()
    ...
    def test_notifier(self, mock_post, mock_isfile):
        ...
        with managed_config(config, "allocations"):
            ...
            my_logger.warning(
                "Hi: %s",
                "a",
                extra={'extra': {'other': 'other', 'site': 'unseen'}},
            )
