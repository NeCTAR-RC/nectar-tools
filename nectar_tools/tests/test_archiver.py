...
class ImageArchiverTests(test.TestCase):
    ...
    def test_restrict_resources_not_ready(self):
        image = fakes.FakeImage(visibility='private', owner='123')
        project = fakes.FakeProject(project_id='123')
        ia = archiver.ImageArchiver(
            resources={'project': project, 'image': image})
    ...
    def test_delete_resources_multiple_images(self):
        project = fakes.FakeProject(project_id='123')
        pia = archiver.ImageArchiver(resources={'project': project})
        ...
        with mock.patch.object(pia, 'g_client') as mock_image:
            ...
            mock_image.images.delete.assert_called_once_with(image1.id)

    def test_restrict_resources_multiple_images(self):
        project = fakes.FakeProject(project_id='123')
        pia = archiver.ImageArchiver(resources={'project': project})
        ...
        with mock.patch.object(pia, 'g_client') as mock_image:
            ...
            mock_image.images.update.assert_called_once_with(
                image2.id, {'visibility': 'private'})


@mock.patch('nectar_tools.auth.get_session', new=mock.Mock())
class ProjectImagesArchiverTests(test.TestCase):

    @mock.patch('nectar_tools.expiry.archiver.ImageArchiver.delete_resources')
    def test_delete_resources(self, mock_super_delete):
        project = fakes.FakeProject(project_id='123')
        ...
        with mock.patch.object(pia, 'g_client') as mock_image:
            ...
            pia.delete_resources(force=True)
            self.assertTrue(pia.images, images)
            mock_super_delete.assert_called_once_with(force=True)

    @mock.patch(
        'nectar_tools.expiry.archiver.ImageArchiver.restrict_resources')
    def test_restrict_resources(self, mock_super_restrict):
        project = fakes.FakeProject(project_id='123')
        pia = archiver.ProjectImagesArchiver(resources={'project': project})
        image1 = fakes.FakeImage(visibility='private', project_id='123')
        image2 = fakes.FakeImage(visibility='public', project_id='123')
        ...
        with mock.patch.object(pia, 'g_client') as mock_image:
            ...
            pia.restrict_resources(force=True)
            self.assertTrue(pia.images, images)
            mock_super_restrict.assert_called_once_with(force=True)
