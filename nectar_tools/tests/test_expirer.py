...
class ExpiryTests(test.TestCase):
    ...
    def test_update_project(self):
        ...
        ex = expirer.Expirer(project, archivers='fake')
...
class AllocationExpiryTests(test.TestCase):
    ...
    def test_set_allocation(self, mock_allocation_session):
    ...
    def test_set_allocation_none(self, mock_allocation_session):
        project = fakes.FakeProject('no-allocation')
        ex = expirer.AllocationExpirer(project)
        self.assertRaises(exceptions.AllocationDoesNotExist, ex.set_allocation)

    def test_process_no_allocation_ignore(self, mock_allocation_session):
        project = fakes.FakeProject('no-allocation')
        ex = expirer.AllocationExpirer(project, force_no_allocation=True)
    ...
    def test_process_allocation_renewed(self, mock_allocation_session):
        project = fakes.FakeProject()
        ex = expirer.AllocationExpirer(project)

        with test.nested(
            mock.patch.object(ex, 'allocation_renewed'),
            mock.patch.object(ex, 'revert_expiry')
        ) as (mock_renewed, mock_revert):
            mock_renewed.return_value = True
            self.assertTrue(ex.process())
            mock_revert.assert_called_once_with()

    def test_process_force_delete(self, mock_allocation_session):
    ...
    def test_process_admin(self, mock_allocation_session):
        project = fakes.FakeProject(expiry_status='admin')
        ex = expirer.AllocationExpirer(project)
    ...
    def test_process_send_warning_extension(self, mock_allocation_session):
        project = fakes.FakeProject('pending1')
        ex = expirer.AllocationExpirer(project)
    ...
    def test_allocation_ready_for_warning(self, mock_allocation_session):
        project = fakes.FakeProject()
        ex = expirer.AllocationExpirer(project)
    ...
    def test_is_allocation_pending(self, mock_allocation_session):
        project = fakes.FakeProject()
        ex = expirer.AllocationExpirer(project)

        ex.allocation = fakes.ALLOCATIONS['active']
        self.assertFalse(ex.is_allocation_pending())

        ex.allocation = fakes.ALLOCATIONS['pending1']
        self.assertTrue(ex.is_allocation_pending())

        ex.allocation = fakes.ALLOCATIONS['pending2']
        self.assertTrue(ex.is_allocation_pending())

    def test_allocation_renewed(self, mock_allocation_session):
        project = fakes.FakeProject()
        ex = expirer.AllocationExpirer(project)
        ex.allocation = fakes.ALLOCATIONS['active']

        with mock.patch.object(ex, 'get_status',
                            return_value=expiry_states.WARNING):
            self.assertTrue(ex.allocation_renewed())

    def test_allocation_renewed_negative(self, mock_allocation_session):
        project = fakes.FakeProject()
        ex = expirer.AllocationExpirer(project)
        ex.allocation = fakes.ALLOCATIONS['active']
        ...
        with mock.patch.object(ex, 'get_status',
            ...
            self.assertFalse(ex.allocation_renewed())

    def test_revert_expiry(self, mock_allocation_session):
        project = fakes.FakeProject(expiry_status=expiry_states.WARNING,
                                    expiry_next_step=BEFORE,
                                    expiry_ticket_id='20')
        ex = expirer.AllocationExpirer(project)
    ...
    def test_revert_expiry_stopped(self, mock_allocation_session):
        project = fakes.FakeProject(expiry_status=expiry_states.STOPPED,
                                    expiry_next_step=BEFORE,
                                    expiry_ticket_id='20')
        ex = expirer.AllocationExpirer(project)
    ...
    def test_revert_expiry_archiving(self, mock_allocation_session):
        project = fakes.FakeProject(expiry_status=expiry_states.ARCHIVING,
                                    expiry_next_step=BEFORE,
                                    expiry_ticket_id='20')
        ex = expirer.AllocationExpirer(project)
...
class PTExpiryTests(test.TestCase):
    ...
    def test_process_archived_not_next_step_resources(self, mock_session):
        project = FakeProjectWithOwner(expiry_status='archived',
                                       expiry_next_step=AFTER)
        ex = expirer.PTExpirer(project)
        with mock.patch.object(ex, 'delete_resources',
    ...
    def test_process_archived_not_next_step(self, mock_session):
        ...
        ex = expirer.PTExpirer(project)
        with mock.patch.object(ex, 'delete_resources',
                               return_value=False) as mock_delete:
            processed = ex.process()
            mock_delete.assert_called_with()
    ...
    def test_notify_near_limit(self, mock_session):
        ...
        ) as (mock_update_project, mock_notifier):
            ex.notify_near_limit()
            mock_notifier.send_message.assert_called_with('first')
    ...
    def test_notify_at_limit(self, mock_session):
        ...
        with test.nested(
            ...
            mock.patch.object(ex, 'notifier'),
            mock.patch.object(ex, 'archiver')
        ) as (mock_update_project, mock_notifier, mock_archiver):
            ex.notify_at_limit()
            mock_notifier.send_message.assert_called_with('second')
            mock_update_project.assert_called_with(
    ...
    def test_notify_over_limit(self, mock_session):
        ...
        with test.nested(
            ...
            mock_notifier.send_message.assert_called_with('final')
            mock_update_project.assert_called_with(
