...
class AllocationExpiryTests(test.TestCase):
    ...
    @freeze_time('2018-02-01')
    def test_send_warning_with_expiry_date(self):
        project = fakes.FakeProject()
        ex = expirer.AllocationExpirer(project)
        mock_allocations = fakes.FakeAllocationManager()
        ex.allocation = mock_allocations.get_current('active')
        ...
        with test.nested(
            ...
            ex.send_warning(extra_context={'expiry_date': expiry_date})
            mock_update_project.assert_called_with(
                expiry_next_step=next_step_date,
                expiry_status=expiry_states.WARNING)
            mock_notification.assert_called_with(
                'first', extra_context={'expiry_date': next_step_date})
            mock_event.assert_called_once_with(
                'warning', extra_context={'expiry_date': next_step_date})
...
class AllocationInstanceExpiryTests(AllocationExpiryTests):
    ...
    def test_process_active_old_enough(self, mock_instances):
        ...
        with test.nested(
            ...
            mock.patch.object(ex, '_get_extra_context'),
        ) as (mock_send_warning, mock_context):
            mock_context.return_value = 'fake'
            self.assertTrue(ex.process())
            mock_send_warning.assert_called_with(extra_context='fake')
    ...
    def test_process_warning_old_enough(self, mock_instances):
        ...
        mock_instances.return_value = ['fake']
        with test.nested(
            mock.patch.object(ex, 'stop_project'),
            mock.patch.object(ex, '_get_extra_context')
        ) as (mock_stop, mock_context):
            mock_context.return_value = 'fake'
            self.assertTrue(ex.process())
            mock_stop.assert_called_with(send_notification=True,
                                         extra_context='fake')
    ...
    def test_process_archiving_not_old_archived_success(self, mock_instances):
        ...
        with test.nested(
            ...
            mock.patch.object(ex, 'archive_project'),
            mock.patch.object(ex, 'archiver'),
        ) as (mock_context, mock_set_arch, mock_arch_proj, mock_archiver):
            mock_archiver.is_archive_successful.return_value = True
            mock_context.return_value = 'fake'
            self.assertTrue(ex.process())
            mock_arch_proj.assert_not_called()
            mock_set_arch.assert_called_with(extra_context='fake')
