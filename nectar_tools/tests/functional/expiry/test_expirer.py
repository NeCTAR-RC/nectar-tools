...
class PTExpiryTests(test.TestCase):
    ...
    def test_active_ok(self):
        ...
        nova_calls = [mock.call.usage.get(self.project.id,
                                          mock.ANY, mock.ANY)
        ]
    ...
    def test_quota_warning_ok(self):
        ...
        self.project.expiry_status = expiry_states.QUOTA_WARNING
        self.project.expiry_next_step = FUTURE

        self._test_process()

    def test_quota_warning_ready(self):
        """Project in deprecated quota warning state ready for next step
        ...
        keystone_calls = self.get_keystone_calls(
            expiry_states.PENDING_SUSPENSION)
    ...
    def test_pending_suspension_ok(self):
        """Project in deprecated pending suspension state not ready

        Expected: Nothing

        """
        self.project.expiry_status = expiry_states.PENDING_SUSPENSION
        self.project.expiry_next_step = FUTURE

        self._test_process()

    def test_pending_suspension_ready(self):
        """Project in deprecated pending suspension state not ready
        ...
        keystone_calls = self.get_keystone_calls(expiry_states.SUSPENDED)
        nova_calls = [
            mock.call.quotas.update(cores=0, instances=0, ram=0,
                                    tenant_id=self.project.id, force=True),
        ...
        self._test_process(keystone_calls=keystone_calls,
                           fd_calls=fd_calls, nova_calls=nova_calls,
                           neutron_calls=neutron_calls,
                           swift_calls=swift_calls)
