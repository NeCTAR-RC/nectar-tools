...
@mock.patch(
    'nectar_tools.auth.get_designate_client', new=fake_clients.get_designate
)
@mock.patch('nectar_tools.auth.get_warre_client', new=fake_clients.get_warre)
...
@mock.patch(
    'nectar_tools.auth.get_openstacksdk', new=fake_clients.get_openstacksdk
)
...
class ProvisionerTests(test.TestCase):
    ...
    def test_provision_convert_pt(self, mock_get_a_client):
        ...
        old_pt = fakes.FakeProject(
            id='q12w',
            name='pt-123',
            description='abc',
            domain_id='my-domain-id',
        )
        new_pt = fakes.FakeProject(
            id='fr45tg',
            name='pt-123_copy',
            description='abc',
            domain_id='my-domain-id',
        ...
        with mock.patch.object(
            provisioning_manager, 'update_allocation'
            ...
            self.assertEqual(
                start_date.strftime('%Y-%m-%d'), allocation.start_date
            )
            ...
            self.assertIn(
                f'Project Name: {allocation.project_name}', email_body
            )
            self.assertIn(f'Expires: {end_date}', email_body)
            # Count <tr> tags to determine amount of quotas
            self.assertEqual(
                len(allocation.quotas), email_body.count('<tr>') - 1
            )
