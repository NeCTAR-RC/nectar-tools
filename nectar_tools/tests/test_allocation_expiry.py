...
class AllocationExpiryTests(testtools.TestCase):
    ...
    def test_process_warning(self, mock_allocation_session):
        ...
        with mock.patch.object(ex, 'restrict_project') as mock_restrict_project:
            ex.process()
            mock_restrict_project.assert_called_with()
    ...
    def test_process_stopped(self, mock_allocation_session):
        ...
        with test.nested(
            ...
            next_step = (NOW + datetime.timedelta(days=90)).strftime(
                base.DATE_FORMAT)
            mock_update_project.assert_called_with(
                expiry_status=expiry_states.ARCHIVING, next_step=next_step)
    ...
    def test_update_project(self, mock_allocation_session):
        project = fakes.FakeProject()
        ex = expirer.AllocationExpirer(project, now=NOW)
        with mock.patch.object(ex.k_client.projects, 'update') as mock_keystone_update:
            ex._update_project(next_step='2016-02-02', expiry_status='blah')
            mock_keystone_update.assert_called_with(project.id,
                                                    expiry_status='blah',
                                                    next_step='2016-02-02')
    ...
    def test_restrict_project(self, mock_allocation_session):
        project = fakes.FakeProject()
        ex = expirer.AllocationExpirer(project, now=NOW)
        one_month = (NOW + datetime.timedelta(days=30)).strftime(base.DATE_FORMAT)
        ...
        with test.nested(
            ...
            ex.restrict_project()
            mock_update_project.assert_called_with(next_step=one_month,
    ...
    def test_archive_project(self, mock_allocation_session):
        project = fakes.FakeProject(expiry_status=expiry_states.STOPPED)
        ex = expirer.AllocationExpirer(project, now=NOW)
        three_months = (NOW + datetime.timedelta(days=90)).strftime(
            base.DATE_FORMAT)
