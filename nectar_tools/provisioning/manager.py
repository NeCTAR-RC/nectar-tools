...
class ProvisioningManager(object):
    ...
    def get_incorrect_zones(self, allocation, project):
        """Returns list of zones that a project has instances running in
        that it shouldn't based on its allocation home.
        """
        zones = self.get_compute_zones(allocation)
        if not zones:
            return []
        ...
        for instance in instances:
            ...
            if az not in zones and az not in incorrect_zones:
                incorrect_zones.append(az)
        return incorrect_zones
    ...
    def notify_provisioned(self, allocation, is_new_project, project, report):
        ...
        if is_new_project:
            ...
            notification = 'update'
            incorrect_zones = self.get_incorrect_zones(allocation, project)
