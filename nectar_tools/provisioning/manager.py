...
class ProvisioningManager(object):
    ...
    def provision(self, allocation):
        ...
        report = self.quota_report(allocation, html=True, show_current=not is_new_project)
    ...
    def create_project(self, allocation):
        ...
        LOG.info("%s: Created new keystone project %s", allocation.id, project.id)
    ...
    def update_project(self, allocation):
        ...
        LOG.info("%s: Updating keystone project %s", allocation.id, allocation.project_id)
    ...
    def _grant_owner_roles(self, allocation, project):
        ...
        if self.noop:
            LOG.info("%s: Would grant manager and member roles to %s", allocation.id,
                     manager.name)
            return
    ...
    def notify_provisioned(self, allocation, is_new_project, project, report):
        if self.noop:
            LOG.info("%s: Would notify %s", allocation.id, allocation.contact_email)
            return
        if is_new_project:
            notification = 'new'
        else:
    ...
    def convert_trial(self, allocation):
        ...
        if self.noop:
            ...
            LOG.info("%s: Would update %s to be %s", allocation.id, old_pt.name,
    ...
    def quota_report(self, allocation, show_current=True, html=False):
        ...
        if show_current:
            _prefix_dict(self.get_current_nova_quota(allocation), 'nova', current)
            _prefix_dict(self.get_current_cinder_quota(allocation), 'cinder', current)
            _prefix_dict(self.get_current_swift_quota(allocation), 'swift', current)
            _prefix_dict(self.get_current_neutron_quota(allocation), 'neutron', current)
            _prefix_dict(self.get_current_trove_quota(allocation), 'trove', current)
            _prefix_dict(self.get_current_manila_quota(allocation), 'manila', current)

        _prefix_dict(allocation.get_allocated_nova_quota(), 'nova', allocated)
        _prefix_dict(allocation.get_allocated_cinder_quota(), 'cinder', allocated)
        _prefix_dict(allocation.get_allocated_swift_quota(), 'swift', allocated)
        _prefix_dict(allocation.get_allocated_neutron_quota(), 'neutron', allocated)
        _prefix_dict(allocation.get_allocated_trove_quota(), 'trove', allocated)
        _prefix_dict(allocation.get_allocated_manila_quota(), 'manila', allocated)
    ...
    def set_nova_quota(self, allocation):
        ...
        client.quotas.update(tenant_id=allocation.project_id, **allocated_quota)
    ...
    def set_trove_quota(self, allocation):
        ...
        if self.noop:
            LOG.info("%s: Would set Trove Quota: %s", allocation.id, allocated_quota)
            return
        client = auth.get_trove_client(self.ks_session)
    ...
    def set_manila_quota(self, allocation):
        ...
        LOG.info("%s: Set Global Manila Quota %s", allocation.id, allocated_quota)
        for share_type in client.share_types.list():
            client.quotas.delete(tenant_id=allocation.project_id,
                                 share_type=share_type.id)
            type_quotas = {}
    ...
    def set_neutron_quota(self, allocation):
        ...
        if allocated_quota:
            ...
            LOG.info("%s: Set Neutron Quota: %s", allocation.id, allocated_quota)
