...
class AllocationExpirer(base.Expirer):
    ...
    def process(self):
        ...
        next_step = getattr(self.project, 'next_step', None)
        LOG.debug("%s: expiry_status: %s, next_step: %s" % (
            self.project.id, expiry_status, next_step))
        if next_step:
            next_step = datetime.datetime.strptime(next_step, base.DATE_FORMAT)
