...
class PTExpirer(base.Expirer):
    ...
    def project_set_defaults(self):
        ...
        self.project.next_step = getattr(self.project, 'next_step', '')
    ...
    def is_ignored_project(self):
        ...
        if status is None:
            ...
            LOG.debug('Project %s is admin. Will never expire', self.project.id)
            return True
    ...
    def get_next_step_date(self):
        ...
        if not next_step:
            # expires is legacy
            next_step = self.project.expires
            if next_step:
                self._update_project(next_step=self.project.expires, expires='')
                self.project.next_step = self.project.expires

        if not next_step:
            LOG.warning('No "next step" date set')
            return None
        try:
            return datetime.datetime.strptime(next_step, base.DATE_FORMAT)
        except ValueError:
            LOG.error('Invalid next_step date: %s for project %s' %
                      (next_step, self.project.id))
        return None
    ...
    def notify_near_limit(self):
        ...
        LOG.info("%s: Usage is over 80 - setting status "
                 "to quota warning" % self.project.name)
