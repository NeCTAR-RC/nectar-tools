...
class AllocationExpirer(ProjectExpirer):
    ...
    def send_warning(self):
        ...
        next_step_date = self.get_notice_date()
...
class AllocationInstanceExpirer(AllocationExpirer):
    ...
    def _get_notification_context(self):
        ...
        if self.get_status(self.project) == expiry_states.ARCHIVED:
            next_step_date = self.get_next_step_date(
                self.project).strftime(DATE_FORMAT)
        elif self.get_status(self.project) == expiry_states.STOPPED:
            next_step_date = self.make_next_step_date(self.now, days=14)
        else:
    ...
    def process(self):
        ...
        # we should go continue and delete the archives as well.
        if not self.instances and \
           zone_expiry_status not in [expiry_states.ARCHIVING,
