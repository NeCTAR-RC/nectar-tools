...
class Expirer(object):
    ...
    def get_next_step_date(self):
        ...
        if not expiry_next_step:
            LOG.debug('%s: No "next_step" date set', self.project.id)
            return None
        try:
...
class AllocationExpirer(Expirer):
    ...
    def __init__(self, project, ks_session=None, dry_run=False,
        ...
        super(AllocationExpirer, self).__init__(project, archivers,
                                                ks_session, dry_run,
                                                disable_project)
    ...
    def process(self):
        ...
        if self.allocation_renewed():
            self.revert_expiry()
            return True

        if self.is_allocation_pending():
            return False
    ...
    def is_allocation_pending(self):
        allocation_status = self.allocation['status']
        if allocation_status != allocation_states.APPROVED:
            LOG.warn("%s: Allocation pending, state='%s'", self.project.id,
                     allocation_states.STATES[allocation_status])
            return True
        return False

    def allocation_renewed(self):
        """Has allocation been renewed since we last saw it

        Allocation has been renewed if:
        1: allocation status == Approved
        2: expiry status != Active
        3: Allocation is not ready to send a warning
        """
        allocation_status = self.allocation['status']
        expiry_status = self.get_status()
        if allocation_status == allocation_states.APPROVED and \
           expiry_status != expiry_states.ACTIVE and \
           not self.allocation_ready_for_warning():
            return True
        return False
    ...
    def revert_expiry(self):
        ...
        expiry_status = self.get_status()
        if expiry_status in [expiry_states.STOPPED, expiry_states.ARCHIVING]:
...
class PTExpirer(Expirer):
    ...
    def __init__(self, project, ks_session=None, dry_run=False,
        ...
        super(PTExpirer, self).__init__(project, archivers, ks_session,
                                        dry_run, disable_project)
        self.project_set_defaults()
