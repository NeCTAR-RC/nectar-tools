...
class AllocationInstanceExpirer(AllocationExpirer):
    ...
    def __init__(self, project, ks_session=None, dry_run=False,
        ...
        super(AllocationInstanceExpirer, self).__init__(
            project, ks_session=ks_session,
            dry_run=dry_run, force_no_allocation=force_no_allocation,
            force_delete=force_delete, disable_project=disable_project,
            archivers=archivers, template_dir='allocation_instances',
    ...
    def set_project_archived(self):
        twelve_weeks = self.make_next_step_date(self.now, unit=6)
    ...
    def archive_project(self):
        if self.project.zone_expiry_status != expiry_states.ARCHIVING:
            LOG.info("%s: Archiving project", self.project.id)
            four_weeks = self.make_next_step_date(self.now, unit=2)
    ...
    def finish_expiry(self):
        ...
        try:
            self.notifier.finish(
                message="Out-of-zone instances expiry has been resolved",
                ticket_id_name='zone_expiry_ticket_id')
        except Exception:
            pass
