...
class ProjectExpirer(Expirer):
    ...
    def check_archiving_status(self, extra_context={}):
        ...
        if self.archiver.is_archive_successful():
            ...
            if extra_context:
                self.set_project_archived(extra_context=extra_context)
            else:
...
class AllocationExpirer(ProjectExpirer):
    ...
    def send_warning(self, extra_context={}):
        ...
        if 'expiry_date' in extra_context:
    ...
    def stop_project(self, send_notification=False, extra_context={}):
        LOG.info("%s: Stopping project", self.project.id)
        self.archiver.stop_resources()
        if 'expiry_date' in extra_context:
            expiry_date = extra_context['expiry_date']
        else:
            expiry_date = self.make_next_step_date(self.now)
        update_kwargs = {self.STATUS_KEY: expiry_states.STOPPED,
                         self.NEXT_STEP_KEY: expiry_date}
        self._update_project(**update_kwargs)
        self.send_event('stop', extra_context=extra_context)
        if send_notification:
            self._send_notification('final', extra_context=extra_context)

    def set_project_archived(self, send_notification=True, extra_context={}):
        super(AllocationExpirer, self).set_project_archived()
        self.send_event('archived', extra_context=extra_context)
        if send_notification:
            self._send_notification('archived', extra_context=extra_context)
...
class AllocationInstanceExpirer(AllocationExpirer):
    ...
    def _get_extra_context(self):
        if self.get_status(self.project) == expiry_states.ARCHIVING:
            next_step_date = self.get_next_step_date(
                self.project).strftime(DATE_FORMAT)
        else:
            next_step_date = self.make_next_step_date(self.now, days=30)
    ...
    def process(self):
        ...
        if not self.instances:
            if self.project.zone_expiry_status != '' or \
               ...
               self.project.zone_expiry_next_step != '':
                self.finish_expiry(
                    message='Out-of-zone instances expiry has been finished')
            return False
        ...
        if zone_expiry_status == expiry_states.ACTIVE:
            if self.is_too_old(self.allocation.start_date, 60):
                ...
                self.send_warning(extra_context=extra_context)
                return True
            return False
        elif zone_expiry_status == expiry_states.WARNING:
            if self.at_next_step(self.project):
                extra_context = self._get_extra_context()
                self.stop_project(send_notification=True,
            ...
            if self.at_next_step(self.project):
                ...
                self.check_archiving_status(extra_context=extra_context)
            return True
