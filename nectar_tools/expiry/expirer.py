...
class Expirer(object):
    ...
    @staticmethod
    def make_next_step_date(now, fortnight=1):
        # If date within 15 December and 31 January, allow more 16 days
        if (now.month == 12 and now.day >= 15) or now.month == 1:
            next_step_date = now + relativedelta(days=14 * fortnight + 16)
        else:
            next_step_date = now + relativedelta(weeks=2 * fortnight)
...
class AllocationInstanceExpirer(AllocationExpirer):
    ...
    def set_project_archived(self):
        twelve_weeks = self.make_next_step_date(self.now, fortnight=6)
    ...
    def archive_project(self):
        if self.project.zone_expiry_status != expiry_states.ARCHIVING:
            LOG.info("%s: Archiving project", self.project.id)
            four_weeks = self.make_next_step_date(self.now, fortnight=2)
    ...
    def _send_notification(self, stage, extra_context={},
                           ticket_id_name='zone_expiry_ticket_id'):
        if self.allocation.notifications:
            context = self._get_notification_context()
            context.update(extra_context)
            recipient, extras = self._get_recipients()
            self.notifier.send_message(stage, recipient, extra_context=context,
                                       extra_recipients=extras,
                                       ticket_id_name=ticket_id_name)
    ...
    def send_warning(self, stage, zone_expiry_status):
        ...
        if zone_expiry_status == expiry_states.DELETED:
            ...
            next_step_date = self.make_next_step_date(self.now, fortnight=2)
