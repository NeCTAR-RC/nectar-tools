...
class JupyterHubVolumeExpiryCmd(cmd_base.CmdBase):
    def __init__(self):
        super(JupyterHubVolumeExpiryCmd, self).__init__(
            log_filename='jupyterhub-volume-expiry.log')
        ...
        if self.args.pvc_id:
            ...
            pvcs = self.kube_client.list_namespaced_persistent_volume_claim(
                self.kube_ns,
                label_selector='hub.jupyter.org/username').items
        else:
    ...
    def add_args(self):
        super(JupyterHubVolumeExpiryCmd, self).add_args()
        self.parser.description = 'Manage JupyterHub PVCs (Volumes)'
        pvc_group = self.parser.add_mutually_exclusive_group()
        pvc_group.add_argument('-p', '--pvc-id',
                                 help='PVC to process (Kubernetes name)')
        pvc_group.add_argument('--all', action='store_true',
                                 help='Run over all PVCs')
    ...
    def set_admin(self):
        ...
        for pvc in self.pvcs:
            ...
            if status == expiry_states.ADMIN:
                ...
                if self.dry_run:
                    LOG.info("Would set status admin for PVC %s (dry run)",
                             pvc.metadata.name)
                else:
            ...
            if self.valid_pvc(pvc):
                ...
                if offset is None or offset_count > offset:
                    try:
                        ...
                        LOG.exception('Exception processing JupyterHub PVC %s',
                                      pvc.metadata.name)
