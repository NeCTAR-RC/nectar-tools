...
class ExpiryNotifier(notifier.FreshDeskNotifier):
    ...
    def send_message(self, stage, owner, extra_context={},
        ...
        if ticket_id > 0:
            ...
            details = self.render_template(
                'resource-details.tmpl', extra_context)
        ...
        if not self.dry_run:
            if self.resource_type == 'project':
                ...
                self.g_client.images.update(self.resource.id,
                    property="nectar_expiry_ticket_id=%s" % str(ticket_id))
    ...
    def _get_ticket_id(self):
        try:
            try:
                id = getattr(self.resource, 'nectar_expiry_ticket_id')
            except AttributeError:
                id = getattr(self.resource, 'expiry_ticket_id', 0)
            finally:
                return int(id)
        except ValueError:
            return 0
