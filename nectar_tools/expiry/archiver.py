...
class CinderArchiver(Archiver):
    ...
    def __init__(self, project, ks_session, dry_run=False):
...
class NeutronArchiver(Archiver):
    ...
    def delete_resources(self, force=False):
        ...
        security_groups = self.ne_client.list_security_groups(
            tenant_id=self.project.id)['security_groups']
        LOG.debug("%s: Found %s security groups",
                  self.project.id, len(security_groups))
    ...
    def _delete_security_group(self, sg_id):
        if not self.dry_run:
            self.ne_client.delete_security_group(sg_id)
            LOG.info("%s: Deleted security group %s", self.project.id, sg_id)
        else:
            LOG.info("%s: Would delete security group %s",
                     self.project.id, sg_id)
