    ...
    def _delete_neutron_resources(self, name, list_method, delete_method,
        ...
        for r in resources:
            if name == 'security_groups' and r['name'] == 'default':
                ...
                continue
            if name == 'security_group_rules' and \
                r['remote_ip_prefix'] is None and r['protocol'] is None:
                LOG.debug("%s: Skip rule %s from default security group %s",
                         self.project.id, r['id'], r['security_group_id'])
                continue
