...
class ProjectTrialAuditor(base.ProjectAuditor):
    def check_owner(self):
        assignments = self.k_client.role_assignments.list(
            project=self.project, role=CONF.keystone.member_role_id
        )
    ...
    def check_expiry(self):
        ...
        if self._past_next_step(expiry_next_step):
            if expiry_status == expiry_states.DELETED:
                ...
                if allocations:
                    LOG.info(
                        "%s: PT expiry blocked on approval "
                    ...
                    LOG.error(
                        "%s: PT expiry stuck: expiry status %s",
                        self.project.id,
                        expiry_status,
                    )
            elif expiry_status in expiry_states.ALL_STATES:
                LOG.info(
                    "%s: PT expiry in unexpected state: "
                ...
                LOG.warning(
                    "%s: PT expiry overdue in ticket hold: expiry status %s",
                    self.project.id,
                    expiry_status,
                )
            else:
                LOG.error(
                    "%s: PT expiry in unknown state: expiry_status %s",
                    self.project.id,
                    expiry_status,
                )
