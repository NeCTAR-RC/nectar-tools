...
class ProjectAllocationAuditor(base.ProjectAuditor):
    ...
    def check_allocation_id(self):
        ...
        if not allocation_id:
            ...
            try:
                ...
                LOG.error(
                    "%s: Can't find allocation for project", self.project.id
                )
                return
            self.repair(
        ...
        try:
            ...
            LOG.info(
                "%s: Linked allocation (%s) not found",
                self.project.id,
                allocation_id,
            )
            return
        ...
        if allocation.parent_request is not None:
            LOG.error(
                "%s: Allocation link (%s) points to a history record",
                self.project.id,
                allocation_id,
            )
        if not allocation.project_id:
            LOG.error(
                "%s: Linked allocation (%s) has no project_id",
                self.project.id,
                allocation_id,
            )
        elif allocation.project_id != self.project.id:
            LOG.error(
                "%s: Linked allocation (%s)'s project_id is wrong",
                self.project.id,
                allocation_id,
            )
    ...
    def check_expiry(self):
        ...
        if self._past_next_step(expiry_next_step):
            if expiry_status in (
                ...
                if (
                    allocation
                    and allocation.status == allocation_states.UPDATE_PENDING
                ):
                    LOG.info(
                        "%s: Allocation expiry blocked on renewal "
                    ...
                    LOG.error(
                        "%s: Allocation expiry stuck: alloc status %s, "
                        "expiry status %s",
                        self.project.id,
                        allocation.status,
                        expiry_status,
                    ...
                    LOG.error(
                        "%s: Allocation expiry for missing allocation: "
                        "expiry status %s",
                        self.project.id,
                        expiry_status,
                    )
                ...
                LOG.info(
                    "%s: Allocation expiry in unexpected state: "
                ...
                LOG.warning(
                    "%s: Allocation expiry overdue in ticket hold: "
                    "expiry status %s",
                    self.project.id,
                    expiry_status,
                )
    ...
    def check_zone_expiry(self):
        ...
        if not zone_expiry_status or not hasattr(
            self.project, 'compute_zones'
        ):  # was reclassified ...
            return
        ...
        if self._past_next_step(zone_expiry_next_step):
            if zone_expiry_status in (
                ...
                LOG.info(
                    "%s: Instance zone expiry in unexpected state: "
                ...
                LOG.warning(
                    "%s: Instance zone expiry overdue in "
                ...
                LOG.error(
                    "%s: Instance zone expiry in unknown state: "
                    "expiry_status %s",
                    self.project.id,
                    zone_expiry_status,
                )
    ...
    def check_deleted_allocation(self):
        ...
        if (
            allocation.status == allocation_states.DELETED
            and expiry_status != expiry_states.DELETED
        ):
            LOG.info(
                "%s: Live project linked to deleted allocation",
                self.project.id,
            )
        elif (
            allocation.status != allocation_states.DELETED
            ...
            LOG.info(
                "%s: Deleted project linked to live allocation",
                self.project.id,
            )
        ...
        if self.project.enabled:
            if expiry_status == expiry_states.DELETED:
                LOG.info(
                    "%s: Expiry deleted project is not disabled",
                    self.project.id,
                )
            elif allocation.status == allocation_states.DELETED:
                LOG.info(
                    "%s: Project for deleted allocation is not disabled",
                    self.project.id,
                )
        else:
            if allocation.status != allocation_states.DELETED:
                LOG.info(
                    "%s: Disabled project with active allocation",
                    self.project.id,
                )
        ...
        if (
            ...
            if self.dry_run:
                LOG.info(
                    "%s: Would mark allocation %s with expiry "
                    "deleted project as deleted",
                    self.project.id,
                    allocation.id,
                )
            self.repair(
                f"{self.project.id}: Marked allocation "
