...
# This audit finds floating networks that are attached
# to an instance on a different site
    ...
    def check_availability_zone(self):
        ...
        for floating_ip in floating_ips['floatingips']:
            ...
            if net_name is None or AZSite is None or networkSite != AZSite:
                LOG.error("floating_ip_id %s network_name %s network_site "\
                        "%s instance_id %s instance_AZ %s instance_site %s"\
                        , floating_ip_id, net_name, networkSite,\
                        device_id, az, AZSite )  
