...
#This audit finds floating networks that are attached to an instance on a different site
class FloatingIPAuditor(base.ResourceAuditor):
    ...
    def check_availability_zone(self):
        ...
        for floating_ip in floating_ips['floatingips']:
            ...
            #if there is no port id, this floating ip can't be attached to an instance
            if port_id != None:
                port = self.nc.show_port(port_id)
                device_owner = port['port']['device_owner']

                #We are looking for device_owner string that looks like compute:AZ
                if device_owner != None:
                    ...
                    if device_owner_name == "compute":
                        ...
                        networkSite = lookUpTable.NetworkToSite.get(network_name)
                        AZSite = lookUpTable.AZToSite.get(device_owner_az)

                        if network_name == None or AZSite == None or networkSite != AZSite:
