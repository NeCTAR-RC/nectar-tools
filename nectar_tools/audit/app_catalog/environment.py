            ...
            if env.status != 'ready':
                ...
                if updated_at < TIME_DIFF:
                    ...
                    if not packages:
                        packages = 'N/A'
                    LOG.info(
                        'environment %s in state %s for more '
    ...
    def check_environment_deleted_project(self):
        for env in self.mc.environments.list(all_tenants=True):
            ...
            if (
                ...
                LOG.error(
                    "Environment %s belongs to a deleted project %s",
                    env.id,
                    project.id,
                )

                self.repair(
                    f"Removed deleted project environment {env.id}",
