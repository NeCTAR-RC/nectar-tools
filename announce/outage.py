...
def collect_args():
    ...
    parser.add_argument('-st', '--start_time', action='store', type=get_datetime,
			help='Start time of outage (e.g. \'09:00 25-06-2015\')',
                        required=True)
...
def mailout(user_id, user, start_ts, end_ts, tz, zone, dry_run, only_affected):
    ...
    text, html = render_templates(subject, instances, start_ts, end_ts, tz, zone, affected)
...
def main():
    ...
    for uid, user in user_data.iteritems():
        if mailout(uid, user, start_ts, end_ts, args.timezone, zone, args.no_dry_run, args.only_affected):
            sent += 1
