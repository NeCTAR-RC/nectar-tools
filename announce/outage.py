...
def mailout(user_id, user, start_ts, end_ts, tz, zone, dry_run, only_affected, node):
...
def render_templates(subject, instances, start_ts, end_ts, tz, zone, affected, node):
    ...
    for tenant, data in tenant_data.iteritems():
        if args.skip_to_tenantid is not None and not proceed:
            if tenant == args.skip_to_tenantid:
                print 'Found tenant: %s name: %s, resuming' % \
                    (tenant, data['name'])
    ...
    for uid, user in user_data.iteritems():
        if args.skip_to_uid is not None and not proceed:
            if uid == args.skip_to_uid:
                print 'Found user: %s name: %s, resuming sending' % \
                    (uid, user['name'])
                proceed = True
