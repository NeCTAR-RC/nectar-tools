...
def add_vms_to_db(session, servers):
    for server in servers:
        add_vm_to_db(session, server)
...
def suspend_instance(session, instance, vm):
    ...
    update_vm_state(session, vm, instance.status, host)
...
def resume_instance(session, instance, vm):
    ...
    if vm.current_state == 'SUSPENDED' and \
       ...
       vm.original_state == 'ACTIVE':
        try:
            print "Resuming & unlocking %s on %s" % (uuid, host)
            instance.resume()
...
def update_vm_states(session, servers):
    for instance in servers:
        host, uuid, state = get_vm_details(instance)
...
def suspend(session, servers):
    suspend_instances(session, servers)
    update_vm_states(session, servers)


def resume(session, servers):
...
def main():
    ...
    if 'view' in args:
        ...
        if args.action == 'db_update':
            ...
            add_vms_to_db(session, servers)
        elif args.action == 'suspend':
            suspend(session, servers)
        elif args.action == 'resume':
            resume(session, servers)
