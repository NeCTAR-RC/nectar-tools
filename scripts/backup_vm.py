...
def returnVMObj(vm_obj, vm_host):
    ...
    for x in vm_obj:
        ...
        dict_var = {"host" : vm_host, "ec2-id": getXMLData(ctx, "/domain/name"), 
                    "uuid" : getXMLData(ctx, "/domain/uuid"), 
                    "ip" :  getXMLData(ctx, "/domain/devices/interface/filterref/parameter[@name='IP']/@value")}
...
def runExecute(path):
    ...
    conv_raw = subprocess.Popen(['qemu-img', 'convert', '-O', 'raw',
