...
def stage2_instances(client, tenant, dry_run=True):
...
def stage2_volumes(client, tenant, all_volumes, dry_run=True):
...
def stage2_images(glance_client, nova_client, tenant, all_images, dry_run=True):
...
def stage2_objects(auth_url, token, swift_url, tenant, dry_run=True):
...
def stage3_instances(client, tenant, dry_run=True):
...
def stage3_volumes(client, tenant, dry_run=True):
...
def stage3_keystone(client, tenant, status, dry_run=True):
...
if __name__ == '__main__':
    ...
    if args.stage2:
        for tenant in tenants:
            ...
            stage2_instances(nc, tenant, dry_run)
            stage2_volumes(cc, tenant, all_volumes, dry_run)
            # TODO takes so long to find images atm
            # need to find a better way
            #all_images = gc.images.list(owner=tenant)
            #stage2_images(gc, nc, tenant, all_images, dry_run)
            if swift_url:
                stage2_objects(auth_url, token, swift_url, tenant, dry_run)
            print
    if args.stage3:
        for tenant in tenants:
            tenant_obj = kc.tenants.get(tenant)
            ...
            stage3_keystone(kc, tenant, tenant_obj.status, dry_run)
            print
