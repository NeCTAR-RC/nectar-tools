...
def mailout(tenant, tenant_data, target_zone, dry_run):
    ...
    for user in users:
        if affected:
            subject = 'NeCTAR Research Cloud outage affecting your VMs (project: %s)' % tenant_name
        else:
            subject = 'NeCTAR Research Cloud outage (project: %s)' % tenant_name
...
if __name__ == '__main__':
    ...
    for tenant, tenantdata in data.iteritems():
        total += mailout(tenant, tenantdata, args.target_zone, args.no_dry_run)
