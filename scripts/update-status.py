            ...
            if tenant.status == 'admin':
                ...
                if tenant.status != 'first':
                    if not DRY_RUN:
                        ...
                        kc.tenants.update(user.tenantId, expires='', status='first')
            elif cpu_hours <= limit*1.2:
                print tenant.id, 'tenant is over 100% - setting status to ' \
                                  'second and quota to 0'
                if tenant.status != 'second':
                    if not DRY_RUN:
                        ...
                        #send_email(user.email, tenant.name)
                        set_nova_quota(nc, tenant.id, ram=0, instances=0,
                            cores=0, force=True)
                        kc.tenants.update(user.tenantId, expires='', status='second')
            elif cpu_hours > limit*1.2:
                print tenant.id, 'tenant is over 120% - setting status to ' \
                                  'final and quota to 0'
                if tenant.status != 'final':
                    if not DRY_RUN:
                        ...
                        kc.tenants.update(user.tenantId, status='final')
                        #kc.tenants.update(user.tenantId, expires=new_expiry, status='final')
        else:
            print 'No status'
...
def get_data(options, filename=False):
    """ Get a list of UUIDs from either file or database.
...
def set_admin(kc, tenant_ids):
    ...
    for tenant_id in tenant_ids:
        ...
        print 'Setting', tenant.id, 'to status admin'
        if not DRY_RUN:
...
if __name__ == '__main__':
    ...
    config = None
    if args.config:
        config = parse_config('mysql', args.config)
    else:
        config = parse_config('mysql')

    data = get_data(config, args.filename)

    if args.set_admin:
        set_admin(kc, data)
    else:
        # Name user attributes for easy access
        users = [{'id':x, 'start':y} for x,y in data]
        update(kc, nc, users)
