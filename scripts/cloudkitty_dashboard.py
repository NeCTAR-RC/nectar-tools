...
def query_resources(begin=None, end=None):
    ...
    if begin:
        query = query + f" and ( ended_at = None or ended_at >= '{begin}' )"
    if end:
        query = query + f" and started_at <= '{end}'"
...
def query_dataframes(id, start_date, end_date, limit=100, offset=0):
    kwargs = {}
    kwargs['filters'] = {'type': 'instance', 'id': id}
    kwargs['limit'] = limit
    kwargs['begin'] = start_date
    kwargs['end'] = end_date
...
def update_output(start_date, end_date):
    text = f'Start is "{start_date}" and end is "{end_date}"'
    global df_day
    global df_id
    ...
    df_id['flavor'] = df_id['id'].apply(
        find_flavor_from_resources, args=(resources,)
    )
...
def update_graphs(active_cell, start_date, end_date, page_current, page_size):
    if active_cell:
        ...
        return data, f'Instance selected is "{id}"'
    else:
        return None, 0
