...
def parse_config(filename='update-expiry.conf'):
...
def update(kc, nc, user_list, dry_run):
    ...
    for row in user_list:
        user = row[0]
        start = row[1]
        # Needed for testing when reading start date from file
        ...
        try:
            ...
            tenant = kc.tenants.get(tenantid)
            tenants += 1
        ...
        usage = nc.usage.get(tenantid, start, end)
        ...
        if not hasattr(tenant, 'started'):
            ...
            if not dry_run:
                kc.tenants.update(tenantid, started=started, status=None)
        else:
...
if __name__ == '__main__':
    ...
    user_list = get_data(args.filename)

    if args.set_admin:
        admin_set(kc, user_list, args.no_dry_run)
    else:
        update(kc, nc, user_list, args.no_dry_run)
